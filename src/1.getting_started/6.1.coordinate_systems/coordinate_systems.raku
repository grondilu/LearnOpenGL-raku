...
constant $vertex-shader-source   = "6.1.coordinate_systems.vs".IO.slurp;
constant $fragment-shader-source = "6.1.coordinate_systems.fs".IO.slurp;
...
sub MAIN {
  ...
  if GLFW::init() {
    ...
    until GLFW::windowShouldClose($window) {
      ...
      $model = GLM::rotate $model, -55.0°, GLM::vec3(1.0e0, 0.0e0, 0.0e0);
